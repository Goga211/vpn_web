...
func TestLoadDotEnvDoesNotOverrideExistingEnv(t *testing.T) {
	t.Setenv("ACCESS_WEB_TEST_KEEP", "from-shell")

	path := filepath.Join(t.TempDir(), ".env")
	if err := os.WriteFile(path, []byte(`
ACCESS_WEB_TEST_KEEP=from-file
ACCESS_WEB_TEST_PLAIN=value
ACCESS_WEB_TEST_QUOTED="hello world"
ACCESS_WEB_TEST_SINGLE='secret value'
`), 0o600); err != nil {
	...
	if got := os.Getenv("ACCESS_WEB_TEST_KEEP"); got != "from-shell" {
		t.Fatalf("existing env was overwritten: %q", got)
	}
	if got := os.Getenv("ACCESS_WEB_TEST_PLAIN"); got != "value" {
		t.Fatalf("plain env = %q", got)
	}
	if got := os.Getenv("ACCESS_WEB_TEST_QUOTED"); got != "hello world" {
		t.Fatalf("quoted env = %q", got)
	}
	if got := os.Getenv("ACCESS_WEB_TEST_SINGLE"); got != "secret value" {
		t.Fatalf("single quoted env = %q", got)
	}
}

func TestGetCSVEnvTrimsEmptyValues(t *testing.T) {
	t.Setenv("ACCESS_WEB_TEST_CSV", " first,second ,, third ")

	got := getCSVEnv("ACCESS_WEB_TEST_CSV")
